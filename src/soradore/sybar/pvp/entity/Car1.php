...
class Car1 extends Human implements Car{
    private $plugin;
    public $width = 1.2;
    public $height = 0.7;
    ...
    public function iniSkin(){
        $img = @imagecreatefrompng($this->plugin->getDataFolder() . "images/texture_car5.png");
        $skinbytes = "";
        $s = (int)@getimagesize($this->plugin->getDataFolder() . "images/texture_car5.png")[1];
        for($y = 0; $y < $s; $y++){
            for($x = 0; $x < 64; $x++){
        ...
        @imagedestroy($img);
        $this->setSkin(new Skin("Standard_CustomSlim", $skinbytes, "", "geometry.car", file_get_contents($this->plugin->getDataFolder() . "images/car.geo5.json")));
        $this->sendSkin();
    }
    ...
    public function setDriver(Player $driver = null){
        ...
        $driver->getDataPropertyManager()->setVector3(Entity::DATA_RIDER_SEAT_POSITION, new Vector3(0, 0.2, 0));
    ...
    public function setPassenger(Player $passenger = null){
        ...
        $passenger->getDataPropertyManager()->setVector3(Entity::DATA_RIDER_SEAT_POSITION, new Vector3(0, 0.7, -1));
